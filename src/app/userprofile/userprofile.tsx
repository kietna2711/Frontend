...
const UserProfile: React.FC = () => {
    ...
    <div className="form-grid">
      ...
      <div className="form-group">
        ...
        <input
          ...
          name="birthDate"
          value={
            editUser?.profile?.birthDate
              ? editUser.profile.birthDate.slice(0, 10)
              : ""
          }
          onChange={handleProfileEditChange}
        />
  ...
  const handleSave = () => {
    ...
    fetch(
      ...
      .then((res) => {
        if (!res.ok)
          throw new Error("Không thể lấy lại thông tin người dùng sau khi lưu");
        return res.json();
      })
      .then((freshUser) => {
        setUser(freshUser);
  ...
  const handleChangePassword = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    fetch(`https://deploy-nodejs-vqqq.onrender.com/users/change-password`, {
      ...
      .catch((err) => {
        ...
        setChangePassMsg({
          type: "error",
          text: err.message || "Đổi mật khẩu không thành công",
        });
      });
  };
    ...
    <div className="container">
      ...
      <div className="main-content">
        <div className="sidebar">
          ...
          >
            <FontAwesomeIcon icon={faUser} style={{ marginRight: 8 }} /> Thông
            tin cá nhân
          </div>
          <div
          ...
          {!isGoogleUser && (
            ...
            >
              <FontAwesomeIcon icon={faLock} style={{ marginRight: 8 }} /> Đổi
              mật khẩu
            </div>
          )}
          ...
          {currentTab === "profile" && (
            <>
              ...
              {isEditing && (
                <>
                  {isGoogleUser
                    ? renderEditFormGoogle()
                    : renderEditFormNormal()}
                  <div style={{ marginTop: 24 }}>
                    <AddressManager
                      addresses={editUser?.profile?.addresses || []}
                      onUpdateAddresses={handleEditAddresses}
                      onAddAddressSuccess={() =>
                        showMessage.success("Đã thêm địa chỉ mới!")
                      }
                      onDeleteAddressSuccess={() =>
                        showMessage.success("Đã xoá địa chỉ!")
                      }
                      readOnly={false}
                    />
                  </div>
            ...
            <div className="change-password-box">
              ...
              <form onSubmit={handleChangePassword}>
                <div className="form-group">
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      setFieldErrors((f) => ({ ...f, oldPassword: undefined }));
                    }}
                    required
                  />
                ...
                {changePassMsg && (
                  <div
                    style={{
                      color: changePassMsg.type === "error" ? "red" : "green",
                      marginBottom: 8,
                    }}
                  >
                    {changePassMsg.text}
