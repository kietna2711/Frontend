...
const UserOrders: React.FC<UserOrdersProps> = ({ username }) => {
  ...
  const fetchOrders = async () => {
    try {
      const res = await fetch(
        `http://localhost:3000/api/usersProfile/by-username-ordername/${username}`
      );
  ...
  const handleUpdateOrder = async (orderId: string) => {
    try {
      const res = await fetch(
        `http://localhost:3000/api/usersProfile/update-status/${orderId}`,
        {
          method: "PUT",
        }
      );
  ...
  const filteredOrders = orders.filter((order) =>
    shouldShowOrder(order.orderStatus)
  );
            ...
            >
              <div className="order-header">
                <div className="order-info">
                  ...
                  <div className="order-meta">
                    ...
                    <span className="product-total">{products.length}</span> sản
                    phẩm
                  </div>
                </div>
              ...
              <div
                className={`order-body ${
                  visibleOrders[order._id] ? "active" : ""
                }`}
              >
