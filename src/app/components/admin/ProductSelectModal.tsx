...
export default function ProductSelectModal(props: ProductSelectModalProps) {
    ...
    () =>
      products.filter((prod) => {
        ...
        const matchCat = !productCategoryFilter
          ...
          : allValidCategoryIds.includes(prod.categoryId) ||
            (prod.subcategoryId &&
              ...
              allValidCategoryIds.includes(prod.subcategoryId.id));
        let matchPrice = true;
        let price = prod.price ?? 0;
        if (priceFilter !== "all") {
          if (priceFilter === "0-100")
            matchPrice = price >= 0 && price < 100_000;
          else if (priceFilter === "100-500")
            matchPrice = price >= 100_000 && price < 500_000;
          else if (priceFilter === "500-1000")
            matchPrice = price >= 500_000 && price < 1_000_000;
        ...
        if (stockFilter !== "all") {
          ...
          else if (stockFilter === "11-100")
            matchStock = qty >= 11 && qty <= 100;
    ...
    [
      ...
      },
      {
        ...
        render: (_: any, record) => {
          ...
          return qty === 0 ? (
            <span style={{ color: "#ff4d4f", fontWeight: 500 }}>Hết hàng</span>
          ) : (
            qty
          );
        },
  ...
  const handleTableChange: TableProps<
    ProductSelectModalProps["products"][0]
  ...
  return (
    <Modal
      ...
      onCancel={onClose}
      title={
        <span style={{ color: "#e7537a", fontWeight: 700, fontSize: 20 }}>
          Chọn Sản Phẩm
        </span>
      }
      footer={[
