...
const LuckyWheel: React.FC<{ visible: boolean; onClose: () => void }> = ({
  visible,
  onClose,
}) => {
  ...
  useEffect(() => {
    Promise.all([
      fetch("https://backend-nm2q.onrender.com/products").then((res) =>
        res.json()
      ),
      fetch("https://backend-nm2q.onrender.com/vouchers").then((res) =>
        res.json()
      ),
      ...
      const prizes = [
        ...
        { label: "THÊM LƯỢT", color: "#fff", bg: "#f39c12", chance: 0.2 },
  ...
  const spin = async () => {
    ...
    const finalRotate =
      360 * 5 + anglePerPrize * prizeIndex + (350 - anglePerPrize / 2);
    ...
    wheelRef.current.style.transition =
      "transform 0.7s cubic-bezier(.7,.1,.9,.3)";
    wheelRef.current.style.transform = `rotate(-${360 * 2}deg)`;

    setTimeout(() => {
      if (!wheelRef.current) return;
      wheelRef.current.style.transition =
        "transform 3.3s cubic-bezier(.17,.67,.83,.67)";
    ...
    setTimeout(async () => {
      ...
      if (prizes[prizeIndex].bg === "#3498db" && prizes[prizeIndex]._id) {
        ...
        setShowModal(true);
        messageApi.success(
          `Bạn đã trúng sản phẩm "${prizes[prizeIndex].label}"!`
        );
      }
      ...
      if (prizes[prizeIndex].bg === "#e74c3c" && prizes[prizeIndex].code) {
        ...
        if (user?.email) {
          await sendVoucherMail(user.email, prizes[prizeIndex]);
          messageApi.success(
            `Mã giảm giá "${prizes[prizeIndex].label}" đã được gửi về email của bạn!`
          );
        }
      }
    }, 4000);
  ...
  const sendVoucherMail = async (email: string, voucher: any) => {
    await fetch("https://backend-nm2q.onrender.com/users/send-voucher-mail", {
      method: "POST",
  ...
  return (
    <Modal
      ...
      bodyStyle={{
        background:
          "radial-gradient(circle at top, #fffbe6 0%, #ffd6e0 80%, #fff6fa 100%)",
        borderRadius: 32,
      ...
      >
        ...
        >
          ...
          <div
            style={{
              ...
              borderRadius: "50%",
              background:
                "radial-gradient(circle at center, #fffbe6 0%, #ffd700 90%, #b8860b 100%)",
          ...
          >
            ...
            <button
              ...
              style={{
                ...
                borderRadius: "50%",
                background:
                  "radial-gradient(circle at center, #fffbe6 0%, #ffd700 80%, #b8860b 100%)",
            ...
            >
              ...
              <span style={{ fontSize: isMobile ? 12 : 24, marginTop: 2 }}>
                {spinning ? "..." : "QUAY"}
              </span>
            </button>
          </div>
        ...
        >
          <p style={{ marginBottom: 24 }}>
            ...
            <strong style={{ color: "#f39c12" }}>{pendingPrize?.label}</strong>!
          </p>
          <p style={{ marginBottom: 24 }}>
          ...
          <div style={{ display: "flex", justifyContent: "center", gap: 16 }}>
            ...
            <button
              onClick={() => {
                ...
                messageApi.success(
                  `Sản phẩm "${pendingPrize?.label}" đã được thêm vào giỏ hàng với giá 0đ!`
                );
              }}
              style={{
                padding: "10px 24px",
