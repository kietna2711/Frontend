...
export default function UserManagement() {
  ...
  useEffect(() => {
    fetch("https://backend-nm2q.onrender.com/users") // Sửa đúng port backend
      .then((res) => res.json())
      .then((data) => {
  ...
  const handleToggleVisibility = async (idx: number) => {
    ...
    try {
      const res = await fetch(
        `https://backend-nm2q.onrender.com/users/${user._id}/toggle-visibility`,
        {
          method: "PATCH",
      ...
      setUsers((users) =>
        users.map((u, i) =>
          i === idx ? { ...u, visible: data.visible, status: data.status } : u
        )
      );
    } catch (err) {
  ...
  const handleToggleRole = async (userId: string, currentRole: string) => {
    try {
      const res = await fetch(
        `https://backend-nm2q.onrender.com/users/${userId}/role`,
        {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
